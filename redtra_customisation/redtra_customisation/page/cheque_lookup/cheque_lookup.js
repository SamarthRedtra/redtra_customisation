...
class ChequeLookup {
	...
	card(pdc) {
		...
		const invoices = (pdc.invoices || []).map((i) => `<tr><td>${this.link(i.doctype,i.name,i.name,i.has_access)}<br>${this.action(i.doctype,i.name,__('Open invoice'),i.has_access)}</td><td>${this.e(i.doctype)}</td><td>${this.projects(i.projects,i.project)}</td><td>${this.e(i.invoice_reference_no || '—')}</td><td>${this.date(i.posting_date)}</td><td class="text-right">${this.money(i.grand_total ?? i.total_amount,i.currency || currency)}</td><td class="text-right">${this.money(i.outstanding_amount ?? i.pdc_outstanding_amount,i.currency || currency)}</td><td>${this.expense(i,currency)}</td><td class="text-right"><strong>${this.money(i.allocated_amount,i.currency || currency)}</strong></td><td>${this.pill(i.status || 'Linked')}</td></tr>`).join('');
		const source = pdc.source_doctype || 'Post Dated Cheques';
		const source_label = pdc.source_label || __('PDC reference');
		return `<div class="cheque-card"><div class="cheque-head"><div><h3>${this.link(source,pdc.name,pdc.reference_no || pdc.name,true)}</h3><div class="text-muted">${this.e(pdc.party_name || pdc.party || '—')} · ${this.e(pdc.payment_type || '—')}</div></div><div class="cheque-amount">${this.money(pdc.amount,currency)}<br>${this.pill(pdc.status || 'Pending')}</div></div><div class="cheque-grid">${this.kv(__('Cheque date'),this.date(pdc.reference_date))}${this.kv(__('Posting date'),this.date(pdc.posting_date))}${this.kv(__('Company'),pdc.company || '—')}${this.kvHtml(__('Project'),this.projects(pdc.projects,pdc.project))}${this.kv(__('Payment mode'),pdc.mode_of_payment || '—')}${this.kv(__('Bank account'),pdc.bank_account || '—')}${this.kv(__('Party type'),pdc.party_type || '—')}${this.kv(source_label,pdc.name)}</div><div class="cheque-section">${this.payment(pdc.payment_entry,currency)}</div><div class="cheque-section"><h4>${__('Linked invoices')} (${(pdc.invoices || []).length})</h4>${invoices ? `<div class="table-responsive"><table class="table cheque-table"><thead><tr><th>${__('Invoice')}</th><th>${__('Type')}</th><th>${__('Project')}</th><th>${__('External Ref.')}</th><th>${__('Date')}</th><th class="text-right">${__('Total')}</th><th class="text-right">${__('Outstanding')}</th><th>${__('Expense')}</th><th class="text-right">${__('Allocated')}</th><th>${__('Status')}</th></tr></thead><tbody>${invoices}</tbody></table></div>` : `<div class="text-muted">${__('No invoices are linked to this cheque.')}</div>`}</div>${pdc.notes ? `<div class="cheque-section"><h4>${__('Notes')}</h4><div class="text-muted">${this.e(pdc.notes)}</div></div>` : ''}</div>`;
	}

	payment(p,currency) { if (!p) return `<h4>${__('Payment entry')}</h4><div class="text-muted">${__('This post-dated cheque has not been converted to a Payment Entry yet.')}</div>`; if (!p.has_access) return `<h4>${__('Payment entry')}</h4><div class="text-muted">${__('A converted Payment Entry exists, but you do not have permission to view it.')}</div>`; return `<h4>${__('Payment entry')} <span class="cheque-pill converted">${__('Converted')}</span></h4><div class="cheque-payment"><div>${this.link('Payment Entry',p.name,p.name,true)}<br><span class="text-muted">${this.e(p.mode_of_payment || '—')} · ${__('Payment date')}: ${this.date(p.posting_date)}</span><br><span class="text-muted">${__('Reference')}: ${this.e(p.reference_no || '—')} · ${__('Reference date')}: ${this.date(p.reference_date)}</span><br><span class="text-muted">${__('From')}: ${this.e(p.paid_from || '—')} · ${__('To')}: ${this.e(p.paid_to || '—')}</span></div><div class="text-right">${this.pill(p.status || '—')}<br><strong>${this.money(p.received_amount || p.paid_amount,currency)}</strong><br>${this.action('Payment Entry',p.name,__('Open payment entry'),true)}</div></div>`; }
	...
	projects(projects, fallback) { const values = [...new Set([fallback, ...(projects || [])].filter(Boolean))]; return values.length ? values.map((project) => `${this.link('Project',project,project,true)}<br>${this.action('Project',project,__('Open project'),true)}`).join('<hr style="margin:5px 0">') : '—'; }
