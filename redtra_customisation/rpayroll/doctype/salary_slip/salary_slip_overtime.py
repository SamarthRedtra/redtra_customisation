...
class SalarySlipOvertime(SalarySlip):
	...
	def set_overtime_details(self):
		if not hasattr(self, "overtime_requests"):
			return
		...
		if payroll_settings and payroll_settings.enable_food_allowance:
...
def _update_payroll_entry_if_no_slips_left(doc):
	...
	if remaining == 0:
		frappe.db.set_value("Payroll Entry", doc.payroll_entry, "salary_slips_created", 0)
